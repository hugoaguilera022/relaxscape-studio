...
async function generateVideoWithProgress(payload,{statusEl=$("#builderStatus"),title="Generando vídeo…"}={}){const box=$("#generationProgress"),bar=$("#generationProgressBar"),pct=$("#generationProgressPercent"),msg=$("#generationProgressMessage"),ttl=$("#generationProgressTitle");if(box)box.classList.remove("hidden");if(ttl)ttl.textContent=title;if(bar)bar.style.width="0%";if(pct)pct.textContent="0%";if(msg)msg.textContent="Iniciando generación…";const started=await api("/api/generate-video",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)});if(!started.jobId)throw Error("El servidor no devolvió el trabajo de generación.");for(let i=0;i<720;i++){const j=await api("/api/generate-video-status?jobId="+encodeURIComponent(started.jobId));const p=Math.max(0,Math.min(100,Number(j.progress)||0));if(bar)bar.style.width=p+"%";if(pct)pct.textContent=p+"%";if(msg)msg.textContent=j.stage||"Generando vídeo…";if(statusEl)statusEl.textContent=(p<100?("🎬 "+(j.stage||"Generando vídeo…")+" "+p+"%"):"✓ Vídeo terminado.");if(j.status==="succeeded"){if(box){bar.style.width="100%";pct.textContent="100%";msg.textContent="Vídeo terminado correctamente.";setTimeout(()=>box.classList.add("hidden"),1200)}return j.result}if(j.status==="failed")throw Error(j.error||"La generación del vídeo falló.");await new Promise(r=>setTimeout(r,1000))}throw Error("La generación está tardando demasiado.");}
...
async function runDailyNow(){const btn=$("#runDailyNow"),el=$("#dailyStatus");if(btn)btn.disabled=true;if(el)el.textContent="🟡 Generando vídeo diario gratuito…";try{const d=await api("/api/daily-video-now",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({durationMinutes:Number($("#scheduleDuration")?.value||60)})});if(!d?.jobId)throw Error("El servidor no devolvió el trabajo de generación.");for(let i=0;i<360;i++){const s=await api("/api/daily-video-status?jobId="+encodeURIComponent(d.jobId));if(s.status==="succeeded"&&s.result){el.innerHTML='🟢 Vídeo terminado: <a href="'+s.result.url+'" target="_blank">Abrir / descargar</a>';await load();return}if(s.status==="failed")throw Error(s.error||"La generación diaria falló.");el.textContent="🟡 "+(s.message||"Generando vídeo…")+" "+(s.progress||0)+"%";await new Promise(r=>setTimeout(r,2000))}}catch(e){if(el)el.textContent="🔴 "+e.message}finally{if(btn)btn.disabled=false}}
