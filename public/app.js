...
async function runDailyNow(){const btn=$("#runDailyNow"),el=$("#dailyStatus");if(btn)btn.disabled=true;if(el)el.textContent="🟡 Generando vídeo diario gratuito…";const started=Date.now();try{const d=await api("/api/daily-video-now",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({durationMinutes:Number($("#scheduleDuration")?.value||60),youtubeMode:true,musicoterapia:true})});if(!d?.jobId)throw Error("El servidor no devolvió el trabajo de generación.");for(let i=0;i<360;i++){const s=await api("/api/daily-video-status?jobId="+encodeURIComponent(d.jobId));if(s.status==="succeeded"&&s.result){el.innerHTML='🟢 Vídeo terminado: <a href="'+s.result.url+'" target="_blank">Abrir / descargar</a>';await load();return}if(s.status==="failed")throw Error(s.error||"La generación diaria falló.");const p=Math.max(0,Math.min(100,Number(s.progress)||0));const elapsed=(Date.now()-started)/1000;const remaining=p>=5&&p<100?Math.max(0,elapsed*(100-p)/p):0;el.textContent="🟡 "+(s.message||"Generando vídeo…")+" · "+p+"%"+(p>=5&&p<100?" · ⏳ Tiempo restante aprox.: "+formatRemaining(remaining):" · ⏳ Calculando tiempo restante…");await new Promise(r=>setTimeout(r,2000))}}catch(e){if(el)el.textContent="🔴 "+e.message}finally{if(btn)btn.disabled=false}}
